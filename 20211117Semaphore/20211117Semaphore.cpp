...
List* createList() {
	...
	pList->pHead = NULL;
	pList->hSema = CreateSemaphore(
			NULL, // default security attributes
			1, // initial count, 맨 처음에 깃발 내려(아무나 들어와)
			1, // maximum count, 최대 1명만 접근 가능
			NULL); // unnamed semaphore, 이름을 붙히지 않는다
	return pList;
}
...
void insertHead(List* pList, Node* newNode) {
	...
	ReleaseSemaphore(pList->hSema, // 세마포어의 핸들
		1, // 하나 증가
		NULL
	); // 깃발 내림
}
